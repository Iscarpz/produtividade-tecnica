...
export function CallDetail({ id, onClose, onRefresh }: { id: number; onClose: () => void; onRefresh: () => void }) {
  ...
  const lastPersistedTechnical = useRef({ diagnostico: "", inspecaoVisual: "" });
  ...
  useEffect(() => {
    if (data?.call) { const technical = { diagnostico: data.call.diagnostico || "", inspecaoVisual: data.call.inspecaoVisual || "" }; setFields({ modelo: data.call.modelo, serial: data.call.serial, queixa: data.call.queixa }); setTechnicalFields(technical); lastPersistedTechnical.current = technical; }
  }, [data?.call?.id, data?.call?.updatedAt]);

  useEffect(() => {
    if (!data?.call || technicalFields.diagnostico === lastPersistedTechnical.current.diagnostico) return;
    const timeout = window.setTimeout(() => {
      lastPersistedTechnical.current = { ...lastPersistedTechnical.current, diagnostico: technicalFields.diagnostico };
      updateTechnicalData.mutate({ id, diagnostico: technicalFields.diagnostico });
    }, 600);
