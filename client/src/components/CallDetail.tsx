...
export function CallDetail({ id, onClose, onRefresh }: { id: number; onClose: () => void; onRefresh: () => void }) {
  ...
  const updateTechnicalData = trpc.calls.updateTechnicalData.useMutation({ onMutate: () => setTechnicalSaveState("saving"), onSuccess: (_result, variables) => { if (variables.diagnostico !== undefined) lastPersistedTechnical.current = { ...lastPersistedTechnical.current, diagnostico: variables.diagnostico }; setTechnicalSaveState("saved"); utils.calls.generateScript.invalidate({ id }); onRefresh(); }, onError: (error) => { setTechnicalSaveState("error"); toast.error(error.message); } });
      ...
      <div className="grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_340px] sm:p-8">
        <main className="min-w-0 space-y-6">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <div className="mt-4"><label className="text-xs font-semibold uppercase tracking-wider text-slate-500" htmlFor="diagnostico">Diagnóstico *</label><Textarea id="diagnostico" aria-label="Diagnóstico" className="mt-2 min-h-24" value={technicalFields.diagnostico} onFocus={() => setDiagnosticEditing(true)} onChange={(event) => setTechnicalFields((current) => ({ ...current, diagnostico: event.target.value }))} onBlur={() => { setDiagnosticEditing(false); saveDiagnostic(); }} placeholder="Descreva o diagnóstico técnico constatado."/></div>
