...
export function CallDetail({ id, onClose, onRefresh }: { id: number; onClose: () => void; onRefresh: () => void }) {
  ...
  const [technicalFields, setTechnicalFields] = useState({ diagnostico: "", inspecaoVisual: "" });
  ...
  const updateTechnicalData = trpc.calls.updateTechnicalData.useMutation({ onSuccess: () => { toast.success("Dados técnicos atualizados"); utils.calls.generateScript.invalidate({ id }); onRefresh(); }, onError: (error) => toast.error(error.message) });
  const addRepair = trpc.calls.addRepair.useMutation({ onSuccess: () => { toast.success("Registro de reparo adicionado"); setRepair(emptyRepair()); onRefresh(); }, onError: (error) => toast.error(error.message) });
  ...
  useEffect(() => {
    if (data?.call) { setFields({ modelo: data.call.modelo, serial: data.call.serial, queixa: data.call.queixa }); setTechnicalFields({ diagnostico: data.call.diagnostico || "", inspecaoVisual: data.call.inspecaoVisual || "" }); }
  ...
  const runAction = (action: string) => {
    ...
    transition.mutate({ id, action: action as any });
  };
  const continueWithoutLaudo = () => {
    if (!laudoAction) return;
    transition.mutate({ id, action: laudoAction });
    setLaudoAction(null);
  };
  const openLaudoCreator = () => {
    window.open(LAUDO_CREATOR_URL, "_blank", "noopener,noreferrer");
    setLaudoAction(null);
  };
  const save = () => updateData.mutate({ id, ...fields });
  const saveTechnical = () => updateTechnicalData.mutate({ id, diagnostico: technicalFields.diagnostico, inspecaoVisual: technicalFields.inspecaoVisual as any });
  const submitRepair = () => {
  ...
  const copyScript = async () => { if (!scriptPreview?.script) return; await navigator.clipboard.writeText(`ANALISE DO CHAMADO\n${scriptPreview.analysis.map((item) => `- ${item}`).join("\n")}\n---\nSCRIPT PREENCHIDO\n${scriptPreview.script}`); toast.success("Script técnico copiado"); };
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-3 sm:p-6">
    <AlertDialog open={Boolean(laudoAction)} onOpenChange={(open) => { if (!open) setLaudoAction(null); }}>
      <AlertDialogContent className="z-[70] w-[calc(100%-2rem)] max-w-[420px] bg-white p-6 text-slate-900 sm:max-w-[420px]">
        ...
        <AlertDialogFooter className="mt-3 gap-3 sm:justify-end"><AlertDialogCancel className="border-slate-200 bg-white text-slate-700 hover:bg-slate-100" onClick={continueWithoutLaudo}>NÃO</AlertDialogCancel><AlertDialogAction className="bg-[#173f5f] text-white hover:bg-[#102d43]" onClick={openLaudoCreator}>SIM</AlertDialogAction></AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
      ...
      <div className="grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_340px] sm:p-8">
        <main className="min-w-0 space-y-6">
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Wrench className="h-4 w-4 text-[#e09f18]"/><h3 className="font-semibold text-slate-900">Dados do chamado</h3></div>{editing ? <div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => { setEditing(false); setFields({ modelo: call.modelo, serial: call.serial, queixa: call.queixa }); }}>Cancelar</Button><Button size="sm" className="bg-[#173f5f] text-white hover:bg-[#102d43]" disabled={updateData.isPending || !fields.modelo || !fields.serial || !fields.queixa} onClick={save}><Save className="mr-1 h-3.5 w-3.5"/>Salvar</Button></div> : <Button size="sm" variant="outline" onClick={() => setEditing(true)}><Pencil className="mr-1 h-3.5 w-3.5"/>Editar</Button>}</div>
            {editing ? <div className="mt-4 grid gap-3"><div className="grid gap-3 sm:grid-cols-2"><Input aria-label="Modelo" value={fields.modelo} onChange={(event) => setFields({ ...fields, modelo: event.target.value })}/><Input aria-label="Serial" value={fields.serial} onChange={(event) => setFields({ ...fields, serial: event.target.value })}/></div><Textarea aria-label="Queixa formalizada" value={fields.queixa} onChange={(event) => setFields({ ...fields, queixa: event.target.value })}/></div> : <><div className="mt-4 rounded-xl border border-amber-200 border-l-4 border-l-[#e09f18] bg-amber-50/60 p-5"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-800">Queixa</p><p className="mt-2 text-base leading-relaxed text-slate-900">{call.queixa}</p></div>{call.queixaOriginal && call.queixaOriginal !== call.queixa && <details className="mt-3 rounded-lg border border-slate-200 px-4 py-3"><summary className="cursor-pointer text-sm font-medium text-slate-600">Ver queixa original extraída</summary><p className="mt-3 text-sm leading-relaxed text-slate-500">{call.queixaOriginal}</p></details>}</>}
          </section>
          <section className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[#e09f18]"/><h3 className="font-semibold text-slate-900">Diagnóstico e inspeção</h3></div><p className="mt-1 text-xs text-slate-500">Preencha estes dados técnicos antes de gerar o script final.</p><div className="mt-4"><label className="text-xs font-semibold uppercase tracking-wider text-slate-500" htmlFor="diagnostico">Diagnóstico *</label><Textarea id="diagnostico" aria-label="Diagnóstico" className="mt-2 min-h-24" value={technicalFields.diagnostico} onChange={(event) => setTechnicalFields({ ...technicalFields, diagnostico: event.target.value })} placeholder="Descreva o diagnóstico técnico constatado."/></div><fieldset className="mt-5"><legend className="text-xs font-semibold uppercase tracking-wider text-slate-500">Inspeção visual / testes de hardware *</legend><div className="mt-3 space-y-2">{["SEM SINAIS DE MAU USO OU DE ABERTURA PRÉVIA.", "MAU USO CONSTATADO - EQUIPAMENTO COM AVARIAS E/OU DANOS FÍSICOS", "CONSTATADO ABERTURA PRÉVIA POR PESSOAL NÃO AUTORIZADO"].map((option) => <label key={option} className={`flex cursor-pointer items-start gap-3 rounded-lg border p-3 text-sm transition-colors ${technicalFields.inspecaoVisual === option ? "border-[#173f5f] bg-blue-50 text-slate-900" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}><input type="radio" name={`inspecao-${id}`} checked={technicalFields.inspecaoVisual === option} onChange={() => setTechnicalFields({ ...technicalFields, inspecaoVisual: option })} className="mt-0.5 accent-[#173f5f]"/><span>{option}</span></label>)}</div></fieldset><div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Imagem / BIOS</p><p className="mt-1 text-sm font-medium text-slate-800">{scriptPreview?.resolvedCatalog ? `${scriptPreview.resolvedCatalog.tipo}: ${scriptPreview.resolvedCatalog.versao}` : "IMAGEM/BIOS NÃO CADASTRADA"}</p><p className="mt-1 text-xs text-slate-500">{scriptPreview?.resolvedCatalog ? "Versão definida automaticamente pelo modelo do chamado." : "Modelo sem versão de Imagem/BIOS cadastrada."}</p></div><Button size="sm" className="bg-[#173f5f] text-white hover:bg-[#102d43]" disabled={!technicalFields.diagnostico.trim() || !technicalFields.inspecaoVisual || updateTechnicalData.isPending} onClick={saveTechnical}>{updateTechnicalData.isPending ? "Salvando..." : "Salvar dados técnicos"}</Button></div></section>
