  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-3 sm:p-6">
    ...
    <article className="mx-auto my-2 w-full max-w-6xl overflow-hidden rounded-2xl bg-[#f7f9fc] shadow-2xl">
      ...
      <div className="grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_340px] sm:p-8">
        ...
        <aside className="min-w-0"><section className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-[#e09f18]"/><h3 className="font-semibold text-slate-900">Histórico do chamado</h3></div><p className="mt-1 text-xs text-slate-500">Etapas relevantes do fluxo operacional.</p><div className="mt-5 space-y-5 border-l-2 border-slate-100 pl-5">{operationalTimeline.length === 0 ? <p className="text-sm text-slate-400">Nenhuma movimentação operacional registrada.</p> : operationalTimeline.map((event) => <div key={event.id} className="relative"><span className="absolute -left-[26px] top-1 h-3 w-3 rounded-full bg-[#e09f18] ring-4 ring-white"/><p className="text-sm font-medium text-slate-800">{event.evento}</p><p className="mt-1 text-xs text-slate-400">{new Date(event.createdAt).toLocaleString()}</p>{event.statusNovo && <p className="mt-1 text-xs text-slate-500">{event.statusAnterior ? `${event.statusAnterior} → ` : ""}{event.statusNovo}</p>}</div>)}</div></section></aside>
      </div>
    </article>
  </div>;
}
