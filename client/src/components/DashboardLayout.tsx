...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#F7F8F7]">
    <aside onMouseEnter={() => setExpanded(true)} onMouseLeave={() => setExpanded(false)} className={`fixed inset-y-0 left-0 z-40 flex ${expanded ? "w-64" : "w-16"} flex-col border-r border-[#1A1F24] bg-[#0D1117] text-white shadow-xl transition-[width] duration-200 ease-out`}>
      <div className="flex h-16 items-center border-b border-white/10 px-3"><button onClick={() => go("/")} title="Ir para Dashboard" className="flex min-w-0 items-center gap-3 rounded-md text-left outline-offset-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#A3E635]"><div className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-lg bg-[#1A1F24]"><img src="/manus-storage/tecbase-symbol_ba576b33.png" alt="TECBASE" className="h-8 w-8 object-contain"/></div><div className={`min-w-0 transition-opacity duration-150 ${expanded ? "opacity-100" : "pointer-events-none w-0 overflow-hidden opacity-0"}`}><img src="/manus-storage/tecbase-horizontal_6248c029.png" alt="TECBASE" className="h-8 w-36 object-contain object-left"/></div></button></div>
