...
export function LaudoDocument({ data, logos, documentRef }: { data: LaudoVisualData; logos: Logos; documentRef?: React.RefObject<HTMLDivElement | null> }) {
  return <article ref={documentRef} className="mx-auto w-[794px] bg-white p-[42px] text-slate-900" style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
    <header className="border-b-2 border-[#2E7D32] pb-4"><div className="grid grid-cols-[1fr_180px] items-start gap-4"><div className="text-[9px] leading-relaxed text-slate-700"><p className="font-bold text-[11px] text-slate-900">Positivo Tecnologia</p><p>CNPJ: 81.243.735/0019-77</p><p>Rua João Bettega, 5200 - Cidade Industrial</p><p>Curitiba - PR</p><p className="mt-1">Tel: (41) 3316-7500</p></div><div className="grid grid-cols-2 gap-x-2 gap-y-2">{logoItems(logos).map(([brand, url]) => <div key={brand} className="flex h-8 items-center justify-center">{url ? <img src={url} alt={`Logo ${brand}`} className="max-h-7 max-w-full object-contain"/> : <span className="text-center text-[8px] font-bold uppercase tracking-wide text-slate-400">{brand}</span>}</div>)}</div></div><div className="mt-4 text-center"><h1 className="text-[16px] font-bold tracking-[0.04em] text-slate-900">LAUDO TÉCNICO - Nº {data.numeroChamado}</h1><p className="mt-1 text-[9px] text-slate-600">Data de Emissão: {data.dataEmissao ? data.dataEmissao.split("-").reverse().join("/") : "—"}</p></div></header>
    ...
    <footer className="mt-9 flex justify-end border-t border-slate-300 pt-4"><div className="w-56 border-t border-slate-500 pt-2 text-center"><p className="text-[10px] font-bold text-slate-800">{data.responsavelTecnico}</p><p className="mt-0.5 text-[8px] uppercase tracking-[0.12em] text-slate-400">Responsável técnico</p></div></footer>
  </article>;
}

function drawPdfHeader(pdf: jsPDF, pageLabel: string, data: LaudoVisualData) {
  pdf.setTextColor(15, 23, 42); pdf.setFontSize(9); pdf.setFont("helvetica", "bold"); pdf.text("Positivo Tecnologia", 14, 13); pdf.setFont("helvetica", "normal"); pdf.setFontSize(7); pdf.text(["CNPJ: 81.243.735/0019-77", "Rua João Bettega, 5200 - Cidade Industrial", "Curitiba - PR", "Tel: (41) 3316-7500"], 14, 18); pdf.setFontSize(10); pdf.setFont("helvetica", "bold"); pdf.text(`LAUDO TÉCNICO - Nº ${data.numeroChamado}`, 105, 15, { align: "center" }); pdf.setFont("helvetica", "normal"); pdf.setFontSize(7); pdf.text(`Data de Emissão: ${data.dataEmissao ? data.dataEmissao.split("-").reverse().join("/") : "—"}`, 105, 20, { align: "center" }); pdf.setDrawColor(46, 125, 50); pdf.line(14, 27, 196, 27); pdf.setTextColor(71, 85, 105); pdf.setFontSize(7); pdf.text(pageLabel, 14, 32);
}

async function preparePhotoForPdf(url: string, index: number) {
  const response = await fetch(url); if (!response.ok) throw new Error(`Não foi possível carregar a foto ${index + 1} para o PDF.`);
  const blob = await response.blob(); const objectUrl = URL.createObjectURL(blob);
  try { const image = await new Promise<HTMLImageElement>((resolve, reject) => { const item = new Image(); item.onload = () => resolve(item); item.onerror = () => reject(new Error(`Não foi possível renderizar a foto ${index + 1} para o PDF.`)); item.src = objectUrl; }); const canvas = document.createElement("canvas"); canvas.width = image.naturalWidth; canvas.height = image.naturalHeight; canvas.getContext("2d")?.drawImage(image, 0, 0); return { dataUrl: canvas.toDataURL("image/jpeg", 0.92), width: image.naturalWidth, height: image.naturalHeight }; } finally { URL.revokeObjectURL(objectUrl); }
}
...
async function buildLaudoPdf(data: LaudoVisualData, ref: React.RefObject<HTMLDivElement | null>) {
  ...
  for (let index = 0; index < data.fotos.length; index += 1) { const photo = await preparePhotoForPdf(data.fotos[index], index); const [x, y] = positions[index]; const ratio = Math.min(82 / photo.width, 94 / photo.height); const width = photo.width * ratio; const height = photo.height * ratio; pdf.setFillColor(248, 250, 252); pdf.rect(x, y, 84, 101, "F"); pdf.setDrawColor(203, 213, 225); pdf.rect(x, y, 84, 101); pdf.addImage(photo.dataUrl, "JPEG", x + (84 - width) / 2, y + (94 - height) / 2 + 2, width, height); pdf.setTextColor(23, 63, 95); pdf.setFontSize(8); pdf.text(`EVIDÊNCIA ${String(index + 1).padStart(2, "0")}`, x + 4, y + 98); }
