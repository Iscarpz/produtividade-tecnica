...
function DocumentSection({ label, title, children }: { label: string; title: string; children: React.ReactNode }) {
  return <section className="border border-slate-200"><div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50 px-4 py-2"><span className="font-mono text-[9px] font-bold tracking-[0.16em] text-[#b27b10]">{label}</span><h2 className="text-[10px] font-bold tracking-[0.1em] text-slate-800">{title}</h2></div><div className="p-4">{children}</div></section>;
}
...
export function LaudoDocument({ data, logos, documentRef }: { data: LaudoVisualData; logos: Logos; documentRef?: React.RefObject<HTMLDivElement | null> }) {
  return <article ref={documentRef} className="mx-auto w-[794px] bg-white p-[42px] text-slate-900" style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
    <header className="border-y-2 border-[#173f5f] py-4"><div className="grid grid-cols-[1fr_1.1fr] items-center gap-6"><div className="grid grid-cols-4 gap-2">{logoItems(logos).map(([brand, url]) => <div key={brand} className="flex h-9 min-w-0 items-center justify-center border-r border-slate-200 px-1 last:border-r-0">{url ? <img src={url} alt={`Logo ${brand}`} className="max-h-7 max-w-full object-contain"/> : <span className="text-center text-[8px] font-bold uppercase tracking-wide text-slate-400">{brand}</span>}</div>)}</div><div className="border-l border-slate-200 pl-5 text-right"><p className="text-[8px] font-bold uppercase tracking-[0.18em] text-[#b27b10]">Assistência técnica autorizada</p><h1 className="mt-1 text-[22px] font-bold tracking-[0.04em] text-[#173f5f]">LAUDO TÉCNICO</h1><p className="mt-1 text-[9px] text-slate-500">Chamado nº {data.numeroChamado} {data.dataEmissao ? `• Emitido em ${data.dataEmissao.split("-").reverse().join("/")}` : ""}</p></div></div></header>
