...
export default function UsersPage() {
  ...
  const setRole = trpc.users.setRole.useMutation({ onSuccess: () => { refresh(); toast.success("Papel atualizado"); }, onError: (error) => toast.error(error.message) });
  ...
  return <div className="p-6 lg:p-8"><div className="mx-auto max-w-6xl">
    <p className="text-sm font-medium text-[#2E7D32]">Configurações</p>
    <div className="mt-1 flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-3xl font-bold text-slate-900">Usuários</h1><p className="mt-1 text-sm text-slate-500">Convide, autorize e gerencie os acessos dos técnicos.</p></div><Button className="bg-[#2E7D32] text-white hover:bg-[#0D1117]" onClick={() => setShowInvite((value) => !value)}><UserPlus className="mr-2 h-4 w-4"/>Convidar técnico</Button></div>
    ...
    <button className="mt-6 text-sm font-medium text-[#2E7D32] hover:underline" onClick={() => setLocation("/configuracoes")}>Voltar para Configurações</button>
  </div></div>;
}
