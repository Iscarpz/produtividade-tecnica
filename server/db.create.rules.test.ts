...
describe("buildNewCallValues", () => {
  it("preserva cada campo textual nas colunas corretas e cria chamado aberto sem data de finalização", () => {
    ...
    expect(buildNewCallValues(1, input, now)).toEqual({ userId: 1, numeroOs: input.numeroOs, serial: input.serial, modelo: input.modelo, queixa: input.queixa, queixaOriginal: input.queixaOriginal, status: "RECEBIDO", dataEntrada: receipt, dataInicioAndamento: null, dataFinalizacao: null, createdAt: now, updatedAt: now });
  });
});
