...
export async function productivityForTeam(userId: number | undefined, from: Date, to: Date) { const db = await getDb(); if (!db) return { RECEBIDO: 0, FINALIZADO: 0, ENVIADO_PP: 0, ENVIADO_ORCAMENTO: 0, ENVIADO_Zurich: 0, EXCLUIDO: 0 }; const team = await listTeamUsers(); const ids = userId ? team.filter((member) => member.id === userId).map((member) => member.id) : team.map((member) => member.id); if (!ids.length) return { RECEBIDO: 0, FINALIZADO: 0, ENVIADO_PP: 0, ENVIADO_ORCAMENTO: 0, ENVIADO_Zurich: 0, EXCLUIDO: 0 }; const [rows, deleted] = await Promise.all([db.select({ type: productivityEvents.tipoEvento, count: sql<number>`count(*)` }).from(productivityEvents).where(and(inArray(productivityEvents.userId, ids), gte(productivityEvents.createdAt, from), lte(productivityEvents.createdAt, to))).groupBy(productivityEvents.tipoEvento), db.select({ count: sql<number>`count(*)` }).from(callDeletionLogs).where(and(inArray(callDeletionLogs.userId, ids), gte(callDeletionLogs.deletedAt, from), lte(callDeletionLogs.deletedAt, to)))]); return buildProductivitySummary(rows.map((row) => ({ type: row.type, count: Number(row.count) })), Number(deleted[0]?.count || 0)); }
